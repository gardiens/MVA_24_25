    ...
    def draw_event():
        ...
        if dim == 2:
            ...
            if show_neighbors and iteration < max_iter:
                lines = [
                    [data_aligned[:, ind1], ref[:, ind2]]
                    for ind1, ind2 in enumerate(neighbors_list[iteration])
                ]
        ...
        if show_neighbors and iteration < max_iter:
            ax.set_title(
                "Iteration {:d} ===> press right / left to change\nNeighbors ON ===> Press n to change (only in 2D)".format(
                    iteration
                )
            )
        else:
            ax.set_title(
                "Iteration {:d} ===> press right / left to change\nNeighbors OFF ===> Press n to change (only in 2D)".format(
                    iteration
                )
            )
